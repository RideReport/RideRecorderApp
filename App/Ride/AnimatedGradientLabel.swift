...
class AnimatedGradientLabel : UILabel {
    var shouldAnimateContinuously: Bool = false
    ...
    var timingFunction: CAMediaTimingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.easeIn) {
        didSet {
            self.reloadAnimations()
        }
    }
    ...
    func commonInit() {
        ...
        self.maskinglabel.adjustsFontSizeToFitWidth = true
    }
    ...
    func reloadAnimations() {
        DispatchQueue.main.async {
            ...
            if self.isAnimating {
                ...
                let xOriginToValue: CGFloat = self.shouldAnimateContinuously ? 0 : -0.5
                let xOriginFromValue: CGFloat = self.shouldAnimateContinuously ? 1.0 : 1.5
                
                if (self.direction == .leftToRight) {
                    animation.fromValue = NSValue(cgPoint:CGPoint(x: xOriginToValue * proposedRect.size.width, y: proposedRect.midY))
                    animation.toValue = NSValue(cgPoint:CGPoint(x: xOriginFromValue * proposedRect.size.width, y: proposedRect.midY))
                } else {
                    animation.fromValue = NSValue(cgPoint:CGPoint(x: xOriginFromValue * proposedRect.size.width, y: proposedRect.midY))
                    animation.toValue = NSValue(cgPoint:CGPoint(x: xOriginToValue * proposedRect.size.width, y: proposedRect.midY))
                }
