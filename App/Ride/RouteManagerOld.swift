...
class RouteManager : NSObject, CLLocationManagerDelegate {
    ...
    let numberOfActiveTrackingPredictionsToTakeAtShorterInterval = 8
    let numberOfActiveTrackingPredictionsToTakeAtNormalInterval = 20
    ...
    private var currentMotionMonitoringPrediction : Prediction?
    private var currentActiveMonitoringPrediction : Prediction?
    ...
    private func startTrip(predictedActivityType activityType:ActivityType) {
        guard let currentTrip = self.currentTrip, !currentTrip.isOpened else {
            return
        }
...
//        } else {
            ...
            self.currentTrip?.batteryAtStart = Int(UIDevice.current.batteryLevel * 100)
//        }
    ...
    func stopTrip(abort: Bool = false, stoppedManually: Bool = false) {
        ...
        if (abort || stoppedTrip.locations.count <= 6) {
            ...
            stoppedTrip.batteryAtEnd = Int(UIDevice.current.batteryLevel * 100)
    ...
    private func processActiveTrackingLocations(_ locations: [CLLocation]) {
        ...
        for location in locations {
            ...
            if (location.speed >= self.minimumSpeedToContinueMonitoring ||
                ...
                if (self.currentActiveMonitoringPrediction == nil && self.isReadyForActiveTrackingActivityQuery(atDate: location.timestamp)) {
                    self.lastActiveTrackingActivityTypeQueryDate = Date()
                    self.currentActiveMonitoringPrediction = Prediction(trip: self.currentTrip!)
                    
                    sensorComponent.classificationManager.queryCurrentActivityType(forSensorDataCollection: self.currentActiveMonitoringPrediction!) { (sensorDataCollection) -> Void in
                        self.currentActiveMonitoringPrediction = nil
            ...
            if let collection = self.currentActiveMonitoringPrediction, let sensorDataCollectionDate = self.lastActiveTrackingActivityTypeQueryDate {
                if (location.timestamp.timeIntervalSince(sensorDataCollectionDate) > -0.1 ) {
                    // we check to make sure the time of the location is after (or within an acceptable amount before) we started the currentActiveMonitoringPrediction
                    loc.sensorDataCollection = collection
                } else {
                    DDLogVerbose("Location is older than currentActiveMonitoringPrediction, skipping.")
                }
            }
    ...
    private func startMotionMonitoring() {
        ...
        if (self.currentTrip == nil) {
            ...
            if let currentGeofenceLocation = Profile.profile().lastGeofencedLocation {
                let _ = Location(copyingLocation: currentGeofenceLocation)
            }
    ...
    private func isReadyForMotionMonitoringActivityQuery(atDate date: Date)->Bool {
        ...
        guard self.currentMotionMonitoringPrediction == nil else {
            return false
        }
    ...
    private func processMotionMonitoringLocations(_ locations: [CLLocation]) {
        ...
        if (self.isReadyForMotionMonitoringActivityQuery(atDate: self.lastMotionMonitoringLocation!.timestamp)) {
            ...
            self.currentMotionMonitoringPrediction = Prediction(prototrip: self.currentPrototrip!)
            self.numberOfActivityTypeQueriesSinceLastSignificantLocationChange += 1
        
            sensorComponent.classificationManager.queryCurrentActivityType(forSensorDataCollection: self.currentMotionMonitoringPrediction!) {[weak self] (sensorDataCollection) -> Void in
                ...
                strongSelf.currentMotionMonitoringPrediction = nil
                ...
                case .automotive where confidence > 0.8 && averageMovingSpeed >= 4:
                    ...
                    strongSelf.startTrip(predictedActivityType: .automotive)
                ...
                case .cycling where confidence > 0.8 && averageMovingSpeed >= 2:
                    ...
                    strongSelf.startTrip(predictedActivityType: .cycling)
                ...
                case .rail where confidence > 0.8 && averageMovingSpeed >= 3:
                    ...
                    strongSelf.startTrip(predictedActivityType: .rail)
        ...
        if let protoTrip = self.currentPrototrip {
            ...
            for location in locs {
                ...
                if let collection = self.currentMotionMonitoringPrediction {
                    loc.sensorDataCollection = collection
                }
            }
        }
