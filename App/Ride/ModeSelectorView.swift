...
@IBDesignable class ModeSelectorView : UISegmentedControl {
    ...
    func commonInit() {
        ...
        self.setBackgroundImage(imageWithColor(UIColor.clear), for: UIControl.State(), barMetrics: .default)
        self.setBackgroundImage(imageWithColor(ColorPallete.shared.unknownGrey), for: .selected, barMetrics: .default)
        self.setDividerImage(imageWithColor(UIColor.clear), forLeftSegmentState: UIControl.State(), rightSegmentState: UIControl.State(), barMetrics: .default)
        ...
        if #available(iOS 10.0, *) {
            self.feedbackGenerator = UIImpactFeedbackGenerator(style: UIImpactFeedbackGenerator.FeedbackStyle.medium)
    ...
    override func layoutSubviews() {
        ...
        self.setTitleTextAttributes([NSAttributedString.Key.font: UIFont.systemFont(ofSize: self.fontSize)], for: UIControl.State())
        self.setTitleTextAttributes([NSAttributedString.Key.font: UIFont.systemFont(ofSize: self.fontSize), NSAttributedString.Key.foregroundColor:UIColor.black], for: .selected)
    }
