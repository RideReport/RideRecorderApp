...
class RewardsViewController: UIViewController, SKPhysicsContactDelegate, SKSceneDelegate
{
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 10.0, *) {
            self.feedbackGenerator = UIImpactFeedbackGenerator(style: UIImpactFeedbackGenerator.FeedbackStyle.light)
            (self.feedbackGenerator as! UIImpactFeedbackGenerator).prepare()
            
            self.inflateFeedbackGenerator = UIImpactFeedbackGenerator(style: UIImpactFeedbackGenerator.FeedbackStyle.heavy)
    ...
    override func viewWillLayoutSubviews()
    {
        ...
        let fontAttributes = [NSAttributedString.Key.font: UIFont(name: "Helvetica", size: 48)!]
