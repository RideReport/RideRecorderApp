...
@IBDesignable class RideNotificationView : UIView, UIScrollViewDelegate {
    ...
    func commonInit() {
        ...
        clearButton.addTarget(self, action: #selector(RideNotificationView.pressedClearButton), for: UIControl.Event.touchUpInside)
        contentView.addSubview(clearButton)
        
        let shareImage = UIImage(named: "Action")?.withRenderingMode(UIImage.RenderingMode.alwaysTemplate)
        ...
        shareButton.addTarget(self, action: #selector(RideNotificationView.pressedShareButton), for: UIControl.Event.touchUpInside)
        ...
        editButton.addTarget(self, action: #selector(RideNotificationView.pressedEditButton), for: UIControl.Event.touchUpInside)
        ...
        destructiveButton.setTitleColor(UIColor.white, for: UIControl.State())
        destructiveButton.addTarget(self, action: #selector(RideNotificationView.pressedDestructiveButton), for: UIControl.Event.touchUpInside)
        ...
        actionButton.addTarget(self, action: #selector(RideNotificationView.pressedActionButton), for: UIControl.Event.touchUpInside)
    ...
    func reloadUI() {
        ...
        editButton.setTitle(self.editTitle, for: UIControl.State())
        ...
        var appNameSize = appNameLabel.text!.size(withAttributes: [NSAttributedString.Key.font: appNameLabel.font])
        ...
        let dateLabelSize = dateLabel.text!.size(withAttributes: [NSAttributedString.Key.font: dateLabel.font])
        let bodySize = bodyLabel.text!.boundingRect(with: CGSize(width: self.bounds.width - insetLeftBody - insetRight - bodySizeOffset, height: self.bounds.height - insetY - appNameSize.height), options: [NSStringDrawingOptions.usesLineFragmentOrigin, NSStringDrawingOptions.truncatesLastVisibleLine], attributes:[NSAttributedString.Key.font: bodyLabel.font], context: nil).size
        ...
        if self.style != .lockScreenStyle || self.bounds.height > 100 {
            ...
            self.heightConstraint = NSLayoutConstraint(item: self, attribute: NSLayoutConstraint.Attribute.height, relatedBy: NSLayoutConstraint.Relation.equal, toItem: nil, attribute: NSLayoutConstraint.Attribute.notAnAttribute, multiplier: 1.0, constant: newHeight)
