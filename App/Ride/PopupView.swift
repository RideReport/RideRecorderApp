...
@IBDesignable class PopupView : UIButton {
    ...
    var arrowOriginPercentageWidth: CGFloat?
    ...
    private func reloadView() {
        ...
        self.widthConstraint = NSLayoutConstraint(item: self, attribute: NSLayoutConstraint.Attribute.width, relatedBy: NSLayoutConstraint.Relation.equal, toItem: nil, attribute: NSLayoutConstraint.Attribute.notAnAttribute, multiplier: 1.0, constant: newWidth)
    ...
    override func draw(_ rect: CGRect) {
        ...
        var origin = smallerPath.size.width
        if let originPercentage = self.arrowOriginPercentageWidth {
            origin = origin * originPercentage + arrowBaseWidth
        }
        
        if arrowHeight > 0 && arrowBaseWidth > 0 {
            let arrowPoint = CGPoint(x: origin - arrowInset - arrowBaseWidth, y: smallerPath.size.height + strokeWidth)
