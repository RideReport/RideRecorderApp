...
class NotificationViewController: UIViewController, UNNotificationContentExtension {
    ...
    override func viewDidLoad() {
        ...
        mapImageHeightConstraint = NSLayoutConstraint(item: self.mapImageView, attribute: .height, relatedBy: NSLayoutConstraint.Relation.equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant:UIScreen.main.bounds.height - 370) // hard coded because notification content is super crazy about autolayout
    ...
    func didReceive(_ notification: UNNotification) {
        if let rideDescription = notification.request.content.userInfo["description"] as? String,
            let rideLength = (notification.request.content.userInfo["length"] as? NSNumber)?.floatValue ?? notification.request.content.userInfo["length"] as? Float, //some versions of iOS give it to us as a float, some as an NSNumber =/
