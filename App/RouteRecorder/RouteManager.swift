...
public class RouteManager : NSObject, CLLocationManagerDelegate {
    ...
    var stopRouteAndDeliverNotificationBackgroundTaskID : UIBackgroundTaskIdentifier = UIBackgroundTaskIdentifier.invalid
    var locationUpdateBackgroundTaskID : UIBackgroundTaskIdentifier = UIBackgroundTaskIdentifier.invalid
    ...
    public func startup(_ fromBackground: Bool, handler: @escaping ()->Void = {() in }) {
        ...
        if (fromBackground) {
            ...
            if (self.locationUpdateBackgroundTaskID == UIBackgroundTaskIdentifier.invalid) {
                ...
                self.locationUpdateBackgroundTaskID = UIApplication.shared.beginBackgroundTask(expirationHandler: { () -> Void in
                    ...
                    self.locationUpdateBackgroundTaskID = UIBackgroundTaskIdentifier.invalid
                })
            }
        }
    ...
    public func isBikeTripInProgress() -> Bool {
        if let route = self.currentRoute {
            return (!route.isClosed && route.activityType == .cycling)
        }
        else {
            return false
        }
    ...
    public func manuallyStartBikeTrip() {
        ...
        self.currentRoute!.open()
        self.currentRoute!.activityType = .cycling
        self.currentRoute!.requiresManualStopping = true
    ...
    func stopGPSRouteAndEnterBackgroundState(abort: Bool = false, stoppedManually: Bool = false) {
        ...
        if (abort || stoppedRoute.locationCount() <= 6) {
            ...
            if (self.stopRouteAndDeliverNotificationBackgroundTaskID == UIBackgroundTaskIdentifier.invalid) {
                ...
                self.stopRouteAndDeliverNotificationBackgroundTaskID = UIApplication.shared.beginBackgroundTask(expirationHandler: { () -> Void in
                    ...
                    self.stopRouteAndDeliverNotificationBackgroundTaskID = UIBackgroundTaskIdentifier.invalid
                })
            }
    ...
    private func checkForGreenLightTriggers(_ locations: [Location], forRoute route: Route) {
        if let triggerZones = RouteRecorderStore.store().fetchGreenLightTriggerRegions() {
            for zone: TriggerZoneCircularRegion in triggerZones {
                for location in locations {
                    let coordinate = CLLocationCoordinate2D(latitude: location.latitude, longitude: location.longitude)
                    if zone.contains(coordinate), !zone.isWithinRegion {
                        // The user was not already in the region but now is, so trigger the green light
                        zone.isWithinRegion = true
                        if let mostRecentLocations = route.mostRecentLocationsForLightTrigger() {
                            APIClient.shared.triggerGreenLight(for: zone, locations: mostRecentLocations)
                        }
                    }
                    else if !zone.contains(coordinate) {
                        // Only count a zone exit if the user has left the zone by more than the horizontal accuracy of the location
                        let exitZoneWithAccuracyPadding = CLCircularRegion(center: zone.center, radius: zone.radius + location.horizontalAccuracy, identifier: zone.uuid)
                        if !exitZoneWithAccuracyPadding.contains(coordinate) {
                            zone.isWithinRegion = false
                        }
                    }
                }
            }
    ...
    private func processGPSLocations(_ locations:[CLLocation], forRoute route: Route) {
        ...
        considerStoppingRoute: if let mostRecentLocationWithSufficientSpeed = self.mostRecentLocationWithSufficientSpeed, let mostRecentGPSLocation = self.mostRecentGPSLocation {
            guard !route.requiresManualStopping else {
                break considerStoppingRoute
            }
    ...
    private func processLocations(_ locations:[CLLocation]) {
        if let route = self.currentRoute, self.isLocationManagerUsingGPS {
            ...
            if UserDefaults.standard.bool(forKey: "GreenLightsEnabled") {
                var locs: [Location] = []
                for location in locations {
                    let loc = Location(recordedLocation: location, isActiveGPS: true)
                    locs.append(loc)
                }
            
                checkForGreenLightTriggers(locs, forRoute: route)
            }
    ...
    public func pauseTracking(_ untilDate: Date! = nil) {
        ...
        if (untilDate != nil) {
            ...
            reminderNotification.alertBody = "Trip tracking is paused! Would you like to resume logging your bike rides?"
    ...
    private func checkPausedAndResumeIfNeeded()->Bool {
        if (isPaused()) {
            ...
            if (pausedUntilDate != nil && pausedUntilDate!.timeIntervalSinceNow <= 0.0) {
                    ...
                    if UserDefaults.standard.bool(forKey: "DebugVerbosityMode") {
                        let notif = UILocalNotification()
                        notif.alertBody = "🐞 Automatically unpausing trip tracking!"
                        notif.category = "DEBUG_CATEGORY"
                        UIApplication.shared.presentLocalNotificationNow(notif)
                    }
                #endif
    ...
    public func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        defer {
            var oldBackgroundTaskID = UIBackgroundTaskIdentifier.invalid
            if (self.locationUpdateBackgroundTaskID != UIBackgroundTaskIdentifier.invalid) {
    ...
    public func locationManager(_ manager: CLLocationManager, didVisit visit: CLVisit) {
        ...
        if (visit.departureDate == NSDate.distantFuture) {
            ...
            if let route = self.currentRoute {
                ...
                guard !priorRoute.isClosed else {
                    return
                }
